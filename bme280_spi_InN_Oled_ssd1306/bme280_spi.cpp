...
// Indoor Navigation:
#define HUM_OSRS_1X          0x01  // Humidity oversampling x1
#define TEMP_OSRS_2X         0x01  // Temperature oversampling x2 
#define PRES_OSRS_16X        0x05  // Pressure oversampling x16
#define MODE_NORMAL          0x03  // Normal mode
#define FILTER_16            0x04  // Filter coefficient 16
#define T_SB_0_5MS           0x00  // Standby time 0.5ms

void init_indoor_navi() {
    write_register(0xF2, HUM_OSRS_1X);
    
    uint8_t config = (T_SB_0_5MS << 5) | (FILTER_16 << 2);
    write_register(0xF5, config);
    
    uint8_t ctrl_meas = (TEMP_OSRS_2X << 5) | (PRES_OSRS_16X << 2) | MODE_NORMAL;
    write_register(0xF4, ctrl_meas);
...
#warning spi/bme280_spi example requires a board with SPI pins
    ...
    while (1) {
        ...
        temperature = compensate_temp(temperature) / 100.0f;
        pressure = compensate_pressure(pressure);
        humidity = compensate_humidity(humidity) / 1024.0f;

        display.setOrientation(0);

        display.clear();

        std::string str1 = std::to_string(temperature);
        const char* temp = str1.c_str(); 

        std::string str2 = std::to_string(pressure);
        const char* press = str2.c_str(); 

        std::string str3 = std::to_string(humidity);
        const char* hum = str3.c_str(); 


        drawText(&display, font_8x8, "Temp:", 0, 0);;
        drawText(&display, font_8x8, temp, 48, 0);


        drawText(&display, font_8x8, "Press:", 0, 16);
        drawText(&display, font_8x8, press, 48, 16);


        drawText(&display, font_8x8, "Hum:", 0, 32);
        drawText(&display, font_8x8, hum, 48, 32);
